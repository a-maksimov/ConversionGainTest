...
% Макрос \shifttext{20pt}{text} для сдвига объкта
\makeatletter
...
%%% Заполняемые формы c подчеркиванием
% https://tex.stackexchange.com/questions/345568/making-fillable-forms-in-latex-using-textfield-and-checkbox
% https://tex.stackexchange.com/questions/398905/typesetting-fillable-pdf-forms-with-hyperref
\renewcommand{\LayoutTextField}[2]{#1 \raisebox{-1.8pt}{\shifttext{-2mm}{#2}}}
% https://tex.stackexchange.com/questions/101628/underlined-text-field-in-a-fillable-form
\renewcommand{\MakeTextField}[2]{{\vbox to #2{\vfill\hbox to #1{\hrulefill}}}}
% команда для формы \ESKDTextField{имя}{шрифт}{длина}{значение}{лейбл}
\newcommand{\ESKDTextField}[5]{%
	\begin{Form}	
		\mbox{\TextField[name = #1,%
		format = {%
		...
		charsize=#2,bordercolor=,backgroundcolor=,height=#2,width=#3,color=black,borderstyle=U,align=1,value=#4]{#5}}%
	\end{Form}
}

%%% Подписи под линиями
% https://tex.stackexchange.com/questions/55698/text-under-a-line
% Using the new command \tlinefill, the line can fill the rest of the line. The first argument is the text that you want in front of the line and the second argument is the text that you want underneath the line. So, \tlinefill{label}{underset text}{underlined text}
\newcommand\tlinefill[3]{$\underset{\text{#1}}{\text{\underline{\hspace{#2}#3\hspace{#2}}}}$}